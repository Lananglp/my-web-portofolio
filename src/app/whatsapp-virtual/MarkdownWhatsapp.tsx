...
export const NonMemoizedMarkdown = ({ children }: { children: string }) => {
  ...
  const components = {
    code: ({ node, inline, className, children, ...props }: any) => {
      ...
      return !inline && match ? (
        <div className="relative mb-6 pt-[1.2rem]">
            <div className="absolute inset-x-0 top-0 w-[72dvw] md:max-w-[500px] bg-zinc-600 text-zinc-300 rounded-t-xl px-4 py-1">
              <p className="text-sm">{match[1] || 'plaintext'}</p>
            </div>
            <pre
                {...props}
                className={`${className} text-sm w-[72dvw] md:max-w-[500px] overflow-x-scroll bg-zinc-100 p-2 rounded-b-xl mt-2 dark:bg-zinc-800`}
            >
    ...
    table: ({ node, children, ...props }: any) => {
      return (
        <div className="w-[72dvw] md:w-full overflow-x-auto mb-6">
...
export const MarkdownWhatsapp = React.memo(
  NonMemoizedMarkdown,
